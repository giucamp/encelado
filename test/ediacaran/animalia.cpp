...
namespace ediacaran_test
{
    ...
    constexpr auto reflect(Animal ** i_ptr)
    {
        ...
        auto const properties = make_array(
            ...
            make_property<EDIACARAN_ACCESSOR(&this_class::get_name_4, &this_class::set_name_4)>("name_4"),
            make_property<EDIACARAN_ACCESSOR(&this_class::get_name_5, &this_class::set_name_5)>("name_5"),
            make_property<EDIACARAN_CONST_ACCESSOR(&this_class::get_color)>("color")
        );
        auto const actions = ediacaran::make_array(
            make_action<EDIACARAN_FUNC(&this_class::make_sound)>("make_sound")    
        );
        return make_class<this_class>(class_name, properties, actions);
    }
    ...
    public:
        ...
        virtual size_t wing_count() const
        {
            return m_wings.size();
        }
    };
    ...
    constexpr auto reflect(Vertebrate ** i_ptr)
    {
        ...
        auto const properties = make_array(
            make_property<EDIACARAN_DATA(this_class, m_vertebre_count)>("vertebre_count"));
