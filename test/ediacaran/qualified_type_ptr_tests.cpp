...
namespace ediacaran_test
{
	...
	template <typename TYPE>
		void qualified_type_ptr_unit_test_type()
	{
    ...
    void qualified_type_ptr_tests()
	{
		...
		{
            qualified_type_ptr q_type_ptr(get_naked_type<float>(), { CV_Flags::Const | CV_Flags::Volatile, CV_Flags::None, CV_Flags::Volatile });
			ENCELADO_TEST_ASSERT(!q_type_ptr.is_empty());
			...
			ENCELADO_TEST_ASSERT(q_type_ptr == q_type_ptr);
			ENCELADO_TEST_ASSERT(q_type_ptr != qualified_type_ptr());
			ENCELADO_TEST_ASSERT(q_type_ptr == get_type<float volatile * * volatile const >());
		}
		{
			ENCELADO_TEST_ASSERT(qualified_type_ptr(get_naked_type<void>(), {} ) == qualified_type_ptr(get_naked_type<void>(), { CV_Flags::None }));
		}

		{
			qualified_type_ptr q_type_ptr(get_naked_type<void>(), { CV_Flags::Const | CV_Flags::Volatile, CV_Flags::None, CV_Flags::Volatile });
			ENCELADO_TEST_ASSERT(!q_type_ptr.is_empty());
			...
			ENCELADO_TEST_ASSERT(q_type_ptr == q_type_ptr);
			ENCELADO_TEST_ASSERT(q_type_ptr != qualified_type_ptr());
			ENCELADO_TEST_ASSERT(q_type_ptr == get_type<void volatile * * volatile const >());
		}
		...
		ENCELADO_TEST_ASSERT(get_type<const int *>() == get_type<int const *>());
		...
		/* qualified_type_ptr is documented (as implementation note) to be big as two pointers. This static_assert is here
			...
			in the library. */
		static_assert(sizeof(qualified_type_ptr) == sizeof(void*) * 2, "qualified_type_ptr is not big as two pointers, as documented.");
