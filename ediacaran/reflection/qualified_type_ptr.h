...
namespace ediacaran
{
    ...
    template <typename TYPE> constexpr qualified_type_ptr get_type();
    ...
    constexpr inline CV_Flags operator|(
      CV_Flags i_first, CV_Flags i_seconds) noexcept
    {
        using underlying_type = std::underlying_type<CV_Flags>::type;
        return static_cast<CV_Flags>(static_cast<underlying_type>(i_first) |
                                     static_cast<underlying_type>(i_seconds));
    }
    ...
    /** Lightweight value-class holding a pointer to a type_t, a number of indirection levels, and the cv-qualification
        ...
        Use \c get_type<TYPE>() to get a \c qualified_type_ptr from a compile-time type.
    ...
    class qualified_type_ptr final
    {
      public:
        ...
        static_assert(std::numeric_limits<uintptr_t>::radix == 2,
          "uintptr_t is expected to be binary");
        ...
        constexpr size_t indirection_levels() const noexcept
        {
            return m_indirection_levels;
        }
        ...
        constexpr const type_t * final_type() const noexcept
        {
            return m_final_type;
        }
        ...
        {
            return (is_const(i_indirection_level) ? CV_Flags::Const
                                                  : CV_Flags::None) |
                   (is_volatile(i_indirection_level) ? CV_Flags::Volatile
                                                     : CV_Flags::None);
        }
        ...
        /** Constructs a non-empty qualified_type_ptr from a final type and an array of CV_Flags's that specifies the cv-qualifiers of the indirection levels.
            ...
            qualified_type_ptr q_type_ptr_2 = get_type<void volatile * * volatile const >();</tt><br>
        ...
        constexpr qualified_type_ptr(
          const qualified_type_ptr & i_source) noexcept = default;

        /** Assigns from the source qualified_type_ptr */
        constexpr qualified_type_ptr & operator=(
          const qualified_type_ptr & i_source) noexcept = default;
