...
namespace ediacaran
{
    ...
    bool try_parse(qualified_type_ptr & o_dest, char_reader & i_source, char_writer & o_error_dest) noexcept
    {
		...
		size_t indirection_levels = 0; // this variable is not the index of the current IL, but the number of IL's so far
		...
		if (indirection_levels > qualified_type_ptr::s_max_indirection_levels)
		{
			o_error_dest << "Exceeded the maximum number of indirection levels (" << qualified_type_ptr::s_max_indirection_levels << ")";
            return false;
		}
